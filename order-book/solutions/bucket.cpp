...
public:
    ...
    void reallocate_() {
        ...
        for (int i = 0, j = st; j != en; i++, j = add_mod(j, 1)) {
            tmp[i] = arr[j];
        }
    ...
    void push(int x) {
        if (st == add_mod(en, 1)) reallocate_();
    ...
    int size() {
        return add_mod(en, -st);
    }
    void erase(int pos) {
        for (int i = add_mod(st, pos + 1); i != en; i = add_mod(i, 1)) {
            arr[add_mod(i, -1)] = arr[i];
        }
        en = add_mod(en, -1);
    }
};
