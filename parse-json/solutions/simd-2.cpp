...
alignas(32) u_int8_t table[65536][16];
...
inline void init_table() {
    for (int mask = 0; mask < 65536; mask++) {
        ...
        for (int i = 0; i < 16; i++) {
            if (mask & (1 << i)) {
                table[mask][out_idx++] = i;
            }
        }

        for (; out_idx < 16; out_idx++) {
            table[mask][out_idx] = 0xFF;
        }
    }
}
...
inline void update(const __m256i &bit, char *&acc, const char * const&s) {
    ...
    u_int16_t lo_mask = mask & (0xFFFF);
    int lo_cnt = __builtin_popcount(lo_mask);
    __m128i lo_chunk = _mm_loadu_si128((__m128i_u *) s);
    __m128i lo_shuffle_mask = _mm_loadu_si128((__m128i_u *) table[lo_mask]);
    __m128i lo_compressed_chunk = _mm_shuffle_epi8(lo_chunk, lo_shuffle_mask);

    _mm_storeu_si128((__m128i_u *) acc, lo_compressed_chunk);
    acc += lo_cnt;

    u_int16_t hi_mask = mask >> 16;
    int hi_cnt = __builtin_popcount(hi_mask);
    __m128i hi_chunk = _mm_loadu_si128((__m128i_u *) (s + 16));
    __m128i hi_shuffle_mask = _mm_loadu_si128((__m128i_u *) table[hi_mask]);
    __m128i hi_compressed_chunk = _mm_shuffle_epi8(hi_chunk, hi_shuffle_mask);

    _mm_storeu_si128((__m128i_u *) acc, hi_compressed_chunk);
    acc += hi_cnt;
}
